...
def main():
    ...
    train = dsets.MNIST(
        root='~/lijiguo/workspace/data/mnist',
        train=True,
    ...
    def train(epoch):
        ...
        for batch_idx, (x0, x1, labels) in enumerate(train_loader):
            ...
            pred = (dist<1).long().squeeze()
            corrects = (pred.cpu().data == labels.cpu().long().data).sum()
            accu = float(corrects)/float(labels.size()[0])

            #zip_data = zip(output1, output2)
            #for logit in list(zip_data):
            #    pred1 = torch.max(logit[0], 1)[1].data
            #    pred2 = torch.max(logit[1], 1)[1].data
            #    pred = torch.IntTensor(pred1 != pred2)
            #    corrects = 
            ...
            if batch_idx % args.batchsize == 0:
                ...
                took = end - start
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss:{:.6f}\tTook: {:.2f}\tAccu: {:.2f}'.format(
